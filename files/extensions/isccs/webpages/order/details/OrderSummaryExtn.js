		...
		if(hasError){
            ...
            mashupContext, mashupRefObj, mashupRefList, inputData, hasError, data, this);
				...
				if(_scBaseUtils.equals("extn_RetryValidationService",sModelObj) || _scBaseUtils.equals("extn_RetryValidationService",sModelObjItem)){
					_scScreenUtils.showInfoMessageBox(this,"Order submitted for Validation successfully");
					...
					var sStatus = _scModelUtils.getStringValueFromPath("mashupArray.0.mashupInputObject.Order.Status",inputData);
					if(_scBaseUtils.isVoid(sStatus)){
						sStatus = _scModelUtils.getStringValueFromPath("0.mashupInputObject.Order.Status",inputData);
					}
					if(sStatus){
						if(_scBaseUtils.equals(sStatus,"1100.040")){
							_scScreenUtils.showInfoMessageBox(this,"Order marked as Completed successfully");
				...
				for (i=0;length>i; i++){
					...
					if(extnOrderlineProperties){
						...
						for (j=0; len>j ; j++){
							...
							if(!_scBaseUtils.isVoid(sDescribedByCharacteristic) || !_scBaseUtils.isVoid(sDescribedByValue)){
								 var eventDefn = _scBaseUtils.getNewBeanInstance();
...
/*				treeData = treeData.replace(/\\n/g, "\\n")
				...
				if(!_scBaseUtils.isVoid(sTreeOrderType) && _scBaseUtils.equals(sTreeOrderType,"CustomerOrder")){
					...
					_scWidgetUtils.showWidget(this, "extn_Retrybtn");
					_scWidgetUtils.showWidget(this, "extn_MarkAsComplete_btn");
				}
        	}else{
				//hide extn_OrderTreePanel
				_scWidgetUtils.hideWidget(this, "extn_OrderTreePanel", true);
	...
	callGetOrderDetailsWithExtnCustomerOrderNo:function(event,bEvent,ctrl,args){
		...
		if(orderModel){
			...
			var sExtnCustOrderNo = _scModelUtils.getStringValueFromPath("Order.Extn.ExtnCustomerOrderNo",orderModel);
	...
	loadOrderTree:function(treeData){
				...
				if(d){
					if(d.orderType){
						if(d.orderType === "CustomerOrder" ){
							if(d.holdFlag == "Y") {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/redPersonCustLock.png";
							} else {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/redPersonCust.png";
							}
						}else if(d.orderType === "ServiceOrder"){
							if(d.holdFlag == "Y") {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/purpleWheelServiceLock.png";
							} else {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/purpleWheelService.png";
							}
						}else if(d.orderType === "ProductOrder"){
							if(d.holdFlag == "Y") {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/greenProdTagLock.png";
							} else {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/greenProdTag.png";
							}
						}else if(d.orderType === "ResourceOrder"){
							if(d.holdFlag == "Y") {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/blueFileResourceLock.png";
							} else {
								sImageToDisplay = "/extn/isccs/resources/css/icons/images/blueFileResource.png";
							}
						}
					}
				}
		...
		function toggle(d) {
		  if (d && d.orderHeaderKey) {
			...
			//_scModelUtils.setStringValueAtModelPath("Order.OrderNo", "PO_SamSmith_4_DS_ADD_2039.AUTO1", newOrderModel);
			//_scModelUtils.setStringValueAtModelPath("Order.OrderHeaderKey", "201611181422274173440", newOrderModel);
