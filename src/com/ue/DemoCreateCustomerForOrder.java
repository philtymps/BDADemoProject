...
public class DemoCreateCustomerForOrder implements YFSBeforeCreateOrderUE {
	...
	private YFCDocument beforeCreateOrder(YFSEnvironment env, YFCDocument input){
		...
		if (YFCCommon.isVoid(eOrder.getAttribute("BillToID")) && !YFCCommon.isVoid(eOrder.getAttribute("EnterpriseCode"))){
			if (!YFCCommon.isVoid(eOrder.getAttribute("BuyerUserId")) || !YFCCommon.isVoid(eOrder.getAttribute("CustomerEMailID"))){
				getCustomerID(env, eOrder, true);
			}
		}
		if (!YFCCommon.isVoid(eOrder.getAttribute("CustomerEMailID")) && YFCCommon.equals(eOrder.getAttribute("CustomerEMailID"), "robertp@gmail.com")){
	...
	public static String getCustomerID(YFSEnvironment env, YFCElement eOrder, boolean updateOrder){
		...
		if (updateOrder){
			if (!YFCCommon.isVoid(eOrder.getChildElement("PersonInfoBillTo"))){
				...
				YFCElement eCustomerAdditionalAddress = eContact.getChildElement("CustomerAdditionalAddressList", true).createChild("CustomerAdditionalAddress");
				...
				if (!YFCCommon.isVoid(eShipPersonInfo)){
					if (!YFCCommon.equals(eShipPersonInfo.getAttribute("ZipCode"), ePersonInfo.getAttribute("ZipCode"))
							|| !YFCCommon.equals(eShipPersonInfo.getAttribute("State"), ePersonInfo.getAttribute("State"))
							|| !YFCCommon.equals(eShipPersonInfo.getAttribute("AddressLine1"), ePersonInfo.getAttribute("AddressLine1"))
							|| !YFCCommon.equals(eShipPersonInfo.getAttribute("City"), ePersonInfo.getAttribute("City"))
							|| !YFCCommon.equals(eShipPersonInfo.getAttribute("AddressLine2"), ePersonInfo.getAttribute("AddressLine2"))){
						eCustomerAdditionalAddress.setAttribute("IsDefaultShipTo", "N");
						eCustomerAdditionalAddress = eContact.getChildElement("CustomerAdditionalAddressList", true).createChild("CustomerAdditionalAddress");
						eCustomerAdditionalAddress.setAttribute("IsBillTo", "Y");
						eCustomerAdditionalAddress.setAttribute("IsShipTo", "Y");
						eCustomerAdditionalAddress.setAttribute("IsSoldTo", "Y");
				...
				if (DemoCreateCustomerForOrder.isBusinessCustomer(eOrder.getAttribute("EnterpriseCode"))){
					eCustomerAdditionalAddress = eCustomer.getChildElement("CustomerAdditionalAddressList", true).createChild("CustomerAdditionalAddress");
					eCustomerAdditionalAddress.setAttribute("IsBillTo", "Y");
					eCustomerAdditionalAddress.setAttribute("IsShipTo", "Y");
					eCustomerAdditionalAddress.setAttribute("IsSoldTo", "Y");
					eCustomerAdditionalAddress.setAttribute("IsDefaultBillTo", "Y");
				...
				try {
					...
					YFCElement output = YFCDocument.getDocumentFor(l_OutputXml).getDocumentElement();
						...
						eOrder.setAttribute("CustomerContactID", output.getChildElement("CustomerContactList", true).getChildElement("CustomerContact", true).getAttribute("CustomerContactID", " "));
						if (!YFCCommon.isVoid(ePersonInfo.getAttribute("FirstName"))){
	...
	private static Document getManageCustomerTemplate(){
		...
		YFCElement eCustomerContact = eCustomer.getChildElement("CustomerContactList", true).getChildElement("CustomerContact", true);
