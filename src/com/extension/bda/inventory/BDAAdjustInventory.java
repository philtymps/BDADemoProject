...
public class BDAAdjustInventory extends BDAServiceApi implements IBDAService {
	...
	public static synchronized String getNodeTypeForNode(YFSEnvironment env, String node) {
		...
		if (_nodeTypes.containsKey(node)) {
			...
			YFCDocument response = YFCDocument
					.getDocumentFor(callApi(env, dInput.getDocument(), dTemplate.getDocument(), "getShipNodeList"));

			YFCElement eOutput = response.getDocumentElement();
			for (YFCElement eShipNode : eOutput.getChildren()) {
				// System.out.println("Adding key: " + eShipNode.getAttribute("ShipnodeKey") + "
				// :: " + eShipNode.getAttribute("NodeType"));
		...
		if (!YFCCommon.isVoid(input)) {
			...
			if (level == 1) {
				...
				for (YFCElement eItem : eItemList.getChildren()) {
					YFCElement eItemResponse = eResponse.getChildElement("ItemResponses", true)
							.createChild("ItemResponse");
					...
					if (level == 2 || !YFCCommon.equals(eItem.getAttribute("SupplyType"), "ONHAND")
							|| !isStoreAdjustment(env, eItem.getAttribute("ShipNode"))) {
						if (isStoreAdjustment(env, eItem.getAttribute("ShipNode"))) {
							System.out.println("Adjusting inventory for: " + eItem.getAttribute("ItemID") + " :: "
									+ eItem.getAttribute("ShipNode") + " :: " + eItem.getAttribute("SupplyType"));
						}
	...
	/**
	 * Get Level of Integration 1 - GIV 2 - IV 3 - IV + SIM
	 */
	...
	private void createAttributeIfDefined(JSONObject obj, YFCElement ele, String objAttribute, String eleAttribute)
	...
	private void callSIMService(YFSEnvironment env, JSONObject obj, String sShipNode, String sLocation)
			throws JSONException {
		...
		try {
			...
			connection.setRequestProperty("Authorization",
					"Bearer " + getAuthToken(env, getPropertyValue(env, "bda.sim.integration.tenant_id"),
							getPropertyValue(env, "bda.sim.integration.client_id"),
							getPropertyValue(env, "bda.sim.integration.secret")));
	...
	public static void clearInventoryFromStore(YFSEnvironment env, String sShipNode, String sItemID)
			throws JSONException {
		StringBuilder url = new StringBuilder();
		url.append("https://store.supply-chain.ibm.com/");
		url.append(getPropertyValue(env, "bda.sim.integration.tenant_id"));
			...
			if (connection.getResponseCode() == 200) {
				...
				for (int i = 0; i < response.size(); i++) {
					if (response.getJSONObject(i).getDouble("quantity") > 0) {
						callSIMRemoveService(env,
								convertStoreInventoryAdjustment(response.getJSONObject(i).getDouble("quantity")),
								sShipNode, response.getJSONObject(i).getString("locationId"), sItemID);
					}
				}
	...
	private static void callSIMRemoveService(YFSEnvironment env, JSONObject obj, String sShipNode, String sLocation,
			String sItemID) throws JSONException {
		StringBuilder url = new StringBuilder();
		url.append("https://store.supply-chain.ibm.com/");
		url.append(getPropertyValue(env, "bda.sim.integration.tenant_id"));
		url.append("/v1/stores/");
	...
	public static synchronized String getAuthToken(YFSEnvironment env, String tenant, String clientId, String secret)
			throws JSONException {
